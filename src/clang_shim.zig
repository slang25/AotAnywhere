...
//!   PUBLISHAOTCROSS_APPLE_SYSROOT - sysroot with Apple framework/library
...
pub fn main(init: std.process.Init) !void {
    ...
    if (detectMacosTarget(args)) {
        if (host_is_macos) {
            ...
            const sysroot = nonEmpty(init.environ_map.get("PUBLISHAOTCROSS_APPLE_SYSROOT"));
...
const pad_file_name = "publishaotcross-gs-pad.c";
const pad_source =
    \\/* Generated by the PublishAotCross clang shim.
    ...
    \\static volatile char publishaotcross_data_page_pad = 1;
    \\
;
